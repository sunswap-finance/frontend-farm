...
export const usePriceSaltBusdLPBnb = () => {
  ...
  const saltPrice = usePriceSaltBnb()
  const [price, setPrice] = useState({
    busdTokenBalance: new BigNumber(0),
    saltTokenBalance: new BigNumber(0),
    totalSupplyLP: new BigNumber(0),
  })
  ...
  useEffect(() => {
    const fetchPrice = async () => {
      const lpAddress = '0xe5d2A6b132Cf96d58B1342A3eB7b743f8b9843c9' // sun/Busd LP
      const [saltTokenBalanceLP, busdTokenBalanceLP, totalSupply] = await multicall(erc20, [
        {
          address: '0x443eF8083f1b3C882C10C3A1A21645c2aE720c94', // sun
          name: 'balanceOf',
          params: [lpAddress],
        },
        {
      ...
      setPrice({
        busdTokenBalance: busdTokenBalanceLP,
        saltTokenBalance: saltTokenBalanceLP,
        totalSupplyLP: totalSupply,
      })
    }
  ...
  // price salt x salts in LP + price blue x blue in LP / LP tokens
  const saltValue = new BigNumber(price.saltTokenBalance).times(saltPrice)
  const busdValue = new BigNumber(price.busdTokenBalance).times(busdPrice)
  const topValue = saltValue.plus(busdValue)
  const lpPrice = topValue.div(price.totalSupplyLP)
...
export const useTotalValue = (): BigNumber => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < farms.length; i++) {
      const farm = farms[i]
      if (farm.lpTotalInQuoteToken && farm.multiplier !== '0X') {
        let val: BigNumber

        if(farm.isTokenOnly){
          if (farm.lpSymbol === QuoteToken.BNB) {
            val = bnbPrice.times(farm.tokenAmount)
          } else if (farm.lpSymbol === QuoteToken.ETH) {
            val = ethPrice.times(farm.tokenAmount)
          } else if (farm.lpSymbol === "BUSD" || farm.lpSymbol === "USDT") {
            val = (new BigNumber(1)).times(farm.tokenAmount)
          } else if (farm.lpSymbol === "SUN") {
            val = saltPrice.times(farm.tokenAmount)
          } else {
          ...
          if (farm.quoteTokenSymbol === QuoteToken.BNB) {
            ...
            val = (new BigNumber(1)).times(farm.lpTotalInQuoteToken)
          } else {
            val = farm.lpTotalInQuoteToken
          }
        }
          farmsTotalValue = farmsTotalValue.plus(val ?? ZERO)
      }
    }
